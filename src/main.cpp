...
struct ProgramState {
    ...
    glm::vec3 dirLightAmbDiffSpec = glm::vec3(0.1f, 0.1f,0.2f);
};
...
int main() {
    ...
    Shader pmShader("resources/shaders/parallaxMapping.vs", "resources/shaders/parallaxMapping.fs");
    // load models
    // -----------
    //church
    ...
    unsigned int parallaxTexture = loadTexture(FileSystem::getPath("resources/textures/bricks.jpg").c_str());
    unsigned int parallaxTextureNormal = loadTexture(FileSystem::getPath("resources/textures/bricks_normal.jpg").c_str());
    unsigned int parallaxHeightMap  = loadTexture(FileSystem::getPath("resources/textures/bricks_disp.jpg").c_str());
    ...
    pmShader.use();
    pmShader.setInt("diffuseMap", 0);
    pmShader.setInt("normalMap", 1);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // draw scene as normal in multi sampled buffers
        glBindFramebuffer(GL_FRAMEBUFFER, aaframebuffer);
        ...
        //render churc
        glm::mat4 modelChurch = glm::mat4(1.0f);
        modelChurch = glm::translate(modelChurch,glm::vec3(0.0f,-1.0f,0.0f) );
        modelChurch = glm::scale(modelChurch, glm::vec3(0.3f));
        ...
        glm::mat4 modelParallax = glm::mat4(1.0f);
        modelParallax = glm::translate(modelParallax, glm::vec3(0.0f, 0.5f, 0.0));
        modelParallax = glm::scale(modelParallax, glm::vec3(1.5f, 0.5f, 1.5f));
        modelParallax = glm::rotate(modelParallax, glm::radians((float)glfwGetTime() * -20.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0)));
        pmShader.setMat4("model", modelParallax);
        pmShader.setVec3("viewPos", programState->camera.Position);
        pmShader.setVec3("lightPos", programState->camera.Position + glm::vec3(0.0f,2.0f,0.0f));
        pmShader.setFloat("heightScale", 0.01f);

        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, parallaxTexture);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, parallaxTextureNormal);
        ...
        // Resolve MSAA framebuffer into the HDR framebuffer
        glBindFramebuffer(GL_READ_FRAMEBUFFER, aaframebuffer);
        glBindFramebuffer(GL_DRAW_FRAMEBUFFER, hdrFBO); // HDR framebuffer
